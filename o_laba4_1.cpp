...
char* sign_(char a[1001]) {
	...
	if (znak != '-') {
		...
		//free нельзя, иначе не вернёт значение
		return a1;
	}
...
char* dif(char* pa, int lena, char* pb, int lenb, int gm) {
	...
	//free(numa); free(numb); - тут нельзя, есть big и little
	...
	//free(numa); free(numb);
	free(big); free(little);
	//или так, они ссылаются на одну и ту же область памяти
	int l = strlen(answer);
...
char* multiply(char* pa, int lena, char* pb, int lenb, int gm) {
	...
	if (!ans) return NULL;
	//memset(ans, '0', ll);
	for (/*unsigned - лучше*/int i = 0; i < ll; i++) ans[i] = '0';
